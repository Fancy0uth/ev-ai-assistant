...
- [ ] Task 16R3（Terra 修复 round 1 待完成）：在认证前覆盖 Task 全生命周期的 console warning/error/pageerror 门；门禁从 `import.meta.url` 解析 spec，分别约束两个 listener 的唯一注册/终态解绑，且禁止中途清空诊断数组；只允许两个测试文件、无生产改动，并在 logout/401 断言后归零。
