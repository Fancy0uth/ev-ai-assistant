...
export async function registerHealthLoopRoutes(app: FastifyInstance, options: { healthTextProvider?: HealthTextProvider; nutritionDataProvider?: NutritionDataProvider }): Promise<void> {
  app.get('/v1/health-capabilities', async (_request, reply) => {
    const data = [
