...
describe('v0.5 reliability storage migration', () => {
  it('upgrades v17 data additively and exposes no raw Provider payload columns', () => {
    ...
    expect(() =>
      database.prepare('update daily_plan_runs set app_version = ? where id = ?').run('not-a-version', 'v17-run'),
    ).toThrow();
    expect(() =>
      database
        .prepare(
        ...
        .run('invalid-version-provider-call', ownerId, 'not-a-version', createdAt, createdAt),
    ).toThrow();
