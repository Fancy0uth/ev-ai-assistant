...
import { taskResponseSchema, type Task } from '@ev/contracts';
...
async function createScheduledTaskThroughUi(page: Page, title: string): Promise<Task> {
  ...
  const task = taskResponseSchema.parse(await createdResponse.json()).data;
...
test('a UI-origin task is reviewed, generated by the guarded Fake, decided into Today, and reloaded through real detail URLs', async ({ page }, testInfo) => {
  ...
  let excludedTask: Task | null = null;
  ...
  try {
    ...
    const fakeEvidenceBaseline = await readFakeProviderEvidence(runDirectory);
    ...
    excludedTask = await createScheduledTaskThroughUi(page, excludedTaskTitle);
    ...
    expect(await readFakeProviderEvidence(runDirectory)).toEqual(fakeEvidenceBaseline);
    ...
    if (excludedTask) {
      const cancelled = await page.context().request.patch(`/api/core/tasks/${excludedTask.id}`, {
        data: { version: excludedTask.version, status: 'CANCELLED' },
      });
      expect(cancelled.status()).toBe(200);
      const cancelledTask = taskResponseSchema.parse(await cancelled.json()).data;
      expect(cancelledTask).toMatchObject({
        id: excludedTask.id,
        status: 'CANCELLED',
        version: excludedTask.version + 1,
      });
    }
    fakeEvidence = await readFakeProviderEvidence(runDirectory);
