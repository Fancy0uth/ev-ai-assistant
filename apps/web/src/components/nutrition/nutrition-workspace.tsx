...
function stageForDraft(draft: MealDraft): NutritionStage {
  if (draft.state === 'CONFIRMED') return 'CONFIRMED';
  return draft.state === 'MATCHES_READY' ? 'MATCHES_REVIEW' : 'CANDIDATES_REVIEW';
}
...
function failureMessage(error: unknown): string {
  ...
  if (error.status === 503 && error.code === 'HEALTH_TEXT_PROVIDER_NOT_CONFIGURED') return '餐食文本 Provider 尚未配置或待审批；请使用手工候选。';
  if (error.status === 503 && error.code === 'NUTRITION_DATA_PROVIDER_NOT_CONFIGURED') return 'DeepSeek 尚未配置；请先在设置中保存 DeepSeek API Key。';
  if (error.status === 503 && error.code.startsWith('NUTRITION_DATA_')) return '公开营养资料当前不可用；请稍后重试或修改食物名称和单位。';
  if (error.status === 503) return 'Provider 当前不可用，请稍后重试。';
...
export function NutritionWorkspace({ initialDate }: { initialDate: string }) {
  ...
  const [mealTextDisclosureAccepted, setMealTextDisclosureAccepted] = useState(false);
  ...
  async function parseMeal(event: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (!mealTextDisclosureAccepted) {
      setFailure('请先确认会向已配置的 DeepSeek 发送餐食文本。');
      return;
    }
    setFailure(null);
    setIsBusy(true);
  ...
  async function matchDraft(): Promise<void> {
    ...
    try {
      ...
      setStage(stageForDraft(matched.draft));
    } catch (error) {
      setFailure(failureMessage(error));
      setStage('FAILED');
    } finally {
  ...
  async function saveCandidates(candidates: ManualCandidate[]): Promise<void> {
    if (!detail) return;
    setFailure(null);
    setIsBusy(true);
    try {
      const payload = await requestCore(`nutrition/meal-drafts/${detail.draft.id}/revisions`, {
        ...
        body: JSON.stringify({ expectedVersion: detail.draft.version, parentRevisionId: detail.revision.id, operation: 'REPLACE_CANDIDATES', candidates }),
      });
      const revised = mealDraftRevisionResponseSchema.parse(payload).data;
      setDetail((current) => current ? { draft: revised.draft, revision: revised.revision, matches: [], source: null, meal: null } : current);
      setStage(stageForDraft(revised.draft));
    } catch (error) {
      setFailure(failureMessage(error));
      setStage('FAILED');
    } finally {
  ...
  async function openSavedDraft(draftId: string): Promise<void> {
    ...
    try {
      ...
      setStage(stageForDraft(saved.draft));
    } catch (error) {
      setFailure(failureMessage(error));
    } finally {
      setIsBusy(false);
  ...
  return (
    <section className="domain-workspace health-workspace" aria-labelledby="nutrition-heading">
      <header className="domain-workspace__header"><p className="section-kicker">NUTRITION / SOURCED REVIEW</p><h1 id="nutrition-heading">饮食记录</h1><p>先表达一餐，再审阅候选与来源。本地缓存优先；缓存未命中时使用已配置的 DeepSeek 与 Wikipedia 公开资料查询。营养数字只来自已选择的来源快照。</p></header>
      {stage === 'ENTRY' || stage === 'FAILED' ? <form className="domain-card domain-form" onSubmit={(event) => void parseMeal(event)}>
        <div className="domain-card__heading"><Utensils aria-hidden="true" size={19} /><div><h2>输入一餐</h2><p>{localDate} · 解析会向已配置的 DeepSeek 发送餐食文本</p></div></div>
        ...
        <label><input checked={mealTextDisclosureAccepted} onChange={(event) => setMealTextDisclosureAccepted(event.target.checked)} type="checkbox" />我理解解析会向已配置的 DeepSeek 发送餐食文本。</label>
        {failure ? <p className="domain-form__error" role="alert">{failure}</p> : null}
        <button disabled={isBusy || !mealTextDisclosureAccepted} type="submit"><Save aria-hidden="true" size={16} /> {isBusy ? '正在解析…' : '解析候选食物'}</button>
      ...
      {detail ? <MealDraftReview key={detail.revision.id} draft={detail.draft} revision={detail.revision} matches={detail.matches} source={detail.source} meal={detail.meal} busy={isBusy} failure={failure} onMatch={matchDraft} onSaveCandidates={saveCandidates} onSaveSelections={saveSelections} onConfirm={confirmDraft} /> : null}
    </section>
  );
}
