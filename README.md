...
“今天”统一按 `Asia/Shanghai` 计算；页面跨午夜、恢复到前台或提交新任务时会重新确认日期。今日状态使用当天的全部任务计算，不受任务列表接口的每页 100 条上限影响。

创建任务时会暂时禁用输入，写入成功后即可输入下一条。若写入已成功但列表刷新失败，界面会明确提示“已保存”；此时点击“重试”只重新读取数据，不会再次创建任务。
...
端到端测试会启动独立的 Core 与 Web，并且只重置被 Git 忽略的 `data/e2e`。它覆盖首次建号、创建高优先级开发任务、刷新后仍存在、完成后规则解释变化、1024px 导航名称与键盘操作、手机布局，以及退出后接口返回 401。组件与 API 回归另外覆盖超百条任务统计、请求格式错误、并发响应乱序、写入与刷新分别失败，以及跨日和草稿保留。
