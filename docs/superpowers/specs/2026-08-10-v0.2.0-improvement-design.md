...
v0.2.0 只有在以下条件同时满足时才可提交 ready-for-review PR：真实三路由与认证启动正确；Tasks 独立闭环；Agent 四组 API 与未配置 503 语义完整；001→002 保留旧数据；八个 P1 都有回归测试；隔离 E2E 四视口通过；`CHANGELOG.md` 和验收文档存在；没有有效 P0/P1。
