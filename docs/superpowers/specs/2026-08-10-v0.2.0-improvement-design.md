...
- 根因：测试把认证视为质量门之外的准备步骤，并把 console error 当作唯一浏览器诊断类型；任务 mutation 与退出的语义断言未与覆盖全程的浏览器诊断集合绑定。
- 严重级别：P1。
- 处理决定：新增窄切片 Task 16R3，仅允许修改 `apps/web/e2e/owner-task-dashboard.spec.ts` 和一个专用 console-gate 测试文件。该切片必须先 RED 后 GREEN，在首次 `page.goto('/')` 前收集 warning/error/pageerror，且在 logout/401 断言后精确断言集合为空；完成针对 spec 的 E2E 回归和全套 E2E 回归，并以独立提交交付。
