...
PUBLIC_GUIDANCE_GENERAL_ADULT_V1复用现有三个内部动作，额外附NHS/AHA出处和版本化摘要；不修改原目录身份，不伪造external review。画像可选新增generalExerciseScope，旧记录兼容读取；新详细规划必须显式确认GENERAL_ADULT_19_64_V1且通过原疼痛/风险/限制检查。适用声明、候选完整依据和政策加入现有hash/引用链。一般力量目标下慢走仅热身/放松，坐站/墙推仅主训练；公开剂量与工程排程估计分列，生成、修订和确认均确定性重验阶段/剂量/总时长。其他目标暂缺完整来源组合时拒绝，不退回未审核工程候选。公开指导不等于个体医疗审核；远端页面不自动监测更新。具体使用边界见DEPLOYMENT，验收事实只见TASKS。

## 服务器MVP增量：07:00与外部能力装配

07:00及当天首次访问补偿使用已有`LOCAL_RULES`协调器生成待审核的每日Proposal，明确记录触发来源；在同一数据库事务中检查当天Run并生成建议，避免重复领取和中途留下半份自动建议。未确认不修改Event。自动流程不解密密钥、不调用模型；用户显式使用模型增强时仍走既有上下文预览/批准流程。进程不运行时无法按点执行，启动补偿与常驻配置边界见DEPLOYMENT。

课表视觉能力按Owner的DeepSeek凭据状态解析，真正提取时才解密并发送图片。不可读字段保留null并供界面补全；确认included课程时严格要求标题、星期、时间和教学周完整，沿用原有提案确认链。公开学习检索在正常server启动时接入匿名维基百科；只属于公开百科资料，不能标成官方课程来源，后续抓取仍经过现有公开地址与大小校验。

营养首源采用USDA FoodData Central的Foundation/SR Legacy文本数据，许可决定`USDA_FDC_CC0_2026_09_20`依据[官方API指南](https://fdc.nal.usda.gov/api-guide/)的CC0说明。适配器不导入图片，源manifest的SHA-256用于稳定标识适配来源范围，**不是USDA全库文件hash**；每条记录另存内容hash及FDC编号。只接受四项营养素完整、单位可核对的记录，按100g或有明确说明及gramWeight的来源份量提供；缺失值不补零、毫升不假定等于克。中文查询原样发送，匹配不足时允许用户改英文名称并核对食物/烹饪状态/份量，不宣称完整中国食物覆盖。

> 本轮范围更新：按[PRD](PRD.md)最新修订，手机端退出MVP，保留服务器Core/Web及电脑浏览器核心流程。下文关于物理iPhone、移动视口与专为手机的私有访问门禁保留为历史方案，不阻断本轮MVP；既有服务器安全、业务与恢复要求不变。
...
### 16.4 2026-09-18 闭环实现约束

本节是既定FIT-02～04的技术细化，尚非完成声明；项目对话已退役，上段仅保留原规格溯源。保留内部starter的旧schema、来源和引用，新详细计划使用显式WORKOUT_PLAN_V2，不能把第三方动作塞进V1的FIRST_PARTY_INTERNAL字面量。来源事实与审核资格分开存储；无资格依据的动作可浏览但不自动入计划。初始本地完整链路可用starter政策候选，不宣称其经过医学认证或全库已审核。

接口沿用现有Web BFF只转发Core `/v1/` 的边界，新能力使用 `/v1/fitness/planning/*` 子路径及V2 payload，不增加任意API版本代理。旧fitness入口继续兼容；详细计划独立解码、引用表追加，复用Workout/Proposal/Action/TimeRequest/反馈，避免第二套事实系统。计划草案确认只创建待办和时间请求；日程Event仍单独审核。

限量上下文仅含本Owner的目标/训练经验/器械、明确提供的身体信息与限制、当天状态、最近14天最多5条反馈、经选择的FITNESS记忆（最多2000字符），候选最多5个。外发前预览实际payload与hash/版本；创建时重建并比对，变化409要求重审，未同意不解密密钥或请求模型。缺失信息标未知，不推定没有疾病；疼痛/急性风险优先硬阻断，不由模型解除。健康正文和记忆不得进入运行日志。

2026-09-19补充当前状态规则：不可变check-in的version恒定并不代表它仍是最新依据。Owner提交新状态后，旧安全记录不能继续授权生成或确认；最新状态有疼痛/急性风险时明确阻断，更新为其他安全状态时旧预览要求重审。此检查复用于预览、确认发送及后续草稿本地校验，不能仅在前端阻止选择旧记录；相同时间戳需有稳定的新旧顺序。修复与定点证据状态仍只见TASKS。

V2明细区分热身/主训练/放松，包含有界整数的组数、次数或持续时间、休息、估计动作耗时与原因，替代动作同样必须引用候选。确定性核算总时长以及替代后的时长，不能超过用户预算、状态上限或可用时间窗；不生成精确重量处方。生成、编辑、提案、接受均校验来源/资格/版本和排程参数，模型不能自改安全政策。

反馈保存用户真实输入的完成/跳过、起止时间、自述用力和不适，不再硬编码无痛或虚构30分钟。已提交反馈作为事实，可生成带来源ID的有限FITNESS记忆摘要；CAS避免覆盖人工编辑，失败明确显示降级，下次上下文仍可从事实表读取，不能将模型建议当成身体事实。

自动摘要只处理新提交且未被用户取消的反馈来源，不通过重读全部历史重建用户删除的记忆；删除后的旧pending不得复活已删除正文。人工区不重写，新的训练反馈是否写入与旧记忆删除区分处理。最近反馈事实的选择范围在外发预览中独立列明，关闭记忆外发不等于允许隐式外发历史反馈。

自动更新的并发保护不能只比较数字version：删除重建可能再次出现version1。实际记忆写入事务内必须核对先前证明的父revision身份，身份变化则在Markdown投影前回滚该次记忆写入；反馈事实和一次尝试记录仍保留。成功的自动来源证明与该revision在同事务生成，不用提交后重读替代。复用既有事务回调，不新增重试后台或改写用户内容。

DeepSeek使用既有凭据服务、固定服务端endpoint/model、受限JSON与取消/超时/字节边界，启动及配置读取不发请求。V2配置状态不冒充真实调用验证，也不使饮食能力自动变成可用。无配置503，不回退假回复；测试Provider只允许隔离测试入口。真实数据下载与模型调用均遵循本轮另行授权。

#### 闭环接线与展示约定

存储上下文、Provider编排和HTTP/UI分别接线；保留旧手动训练流程。新的详细计划服务复用既有幂等命令、共享每日训练调用预算、Workout修订与提案事务。预览只保存有界的选择/引用，不额外保存整份健康和记忆正文；当前记忆revision UUID进入hash，避免删除后重建同版本误用旧授权。候选技术快照随修订保留以供历史展示，当前资格失效仍能阻止新的确认，历史可读不等于仍可执行。

界面依次为画像与当天状态、有限候选、实际外发预览、详细草稿、训练提案决定、日程审核入口、实际训练反馈。显示动作名称/步骤、三阶段、来源、参数和替代项；发送、接受训练、接受日程是三个独立动作。完成/跳过提交用户填写的时间、用力与不适，不自动声明无痛。反馈已保存但记忆摘要失败时分别展示两个状态，不回滚已确认事实或显示整条成功。

生成配置状态只依据Owner凭据元数据及实际装配，不进行探测请求；Fake标签只在隔离测试入口出现。最终本地验证以合成Owner及Fake详细计划完成一次页面链路：确认前Action/TimeRequest/Event均无新增，接受训练后仅前两者新增，日程确认后才新增Event，反馈后再次预览能看到授权范围内的反馈/记忆。真实效果、真实动作数据审核及真实DeepSeek调用单独标未验证。

## 17. DeepSeek联网营养查询与本地复用（2026-09-20用户修订）

用户明确仅配置DeepSeek，取消USDA密钥作为营养功能前置条件。复用现有候选→来源匹配→人工选择→确定性计算→确认餐食流程；USDA适配器与凭据仅保留为可选专用来源。

营养Provider先按Owner、原始食物名称（NFKC/空白/大小写规范化）、单位和适配版本读取本地缓存。名称中的生熟、品牌、做法不删去、不合并；数量变化按同一基准份量重新计算。命中后从已校验缓存快照读取，不解密密钥、不调用模型或网络，不消耗外部查询额度。有效缓存30天，只有带依据的正向记录可写入，失败及无结果不当作有效食物缓存。

未命中时，DeepSeek返回有界工具调用，Core执行固定公开来源工具，并将真实取得的内容作为工具结果提交给DeepSeek提取。首个无额外密钥来源是中/英文Wikipedia，属于公开百科资料，不是全网搜索或医疗认证；不保证所有中国菜肴、品牌或份量都有结果。模型不能直接提供无来源营养值，网页中的指令不作为系统指令执行。必须提供实际页面中可核对的份量基准和四项营养数值/单位证据；没有完整依据就保持未匹配，不能把克、毫升、个互相猜测转换。

一期使用固定域名的MediaWiki搜索/页面读取，禁止模型提供任意网络目标；限制请求数量、响应字节和总时长。每批未命中最多两次DeepSeek调用（工具请求、根据实际工具结果提取）；只有食物名称和单位用于外发，不发送Owner身份、完整餐食原文或健康画像。本地保留结果、来源URL、检索时间、内容摘要hash及短证据片段，非整页复制；前端提供源链接并要求核对食物、生熟状态与单位。历史已确认餐食保持原来源快照，不因缓存更新改写。

营养查询独立最长60秒，幂等处理租期70秒；其余旧能力仍保持8秒边界及原默认租期。页面展示外发说明，用户点击匹配才执行；没有无交互后台预抓取。达到外部查询额度后仍允许纯缓存命中，缓存缺失继续被额度阻断。统计中的actualCalls为Provider批操作计数，非DeepSeek逐HTTP计费计数；不得将它当账单金额。

验证保持最小：一个源工具/证据/缓存组合场景，一个缓存不消耗额度并能确认的服务场景，受影响Web用例；集中类型/生产构建，获准后一个合成真实查询及重复缓存读取。真实验证状态只见TASKS，任何Mock不替代来源网络和真实模型证据。
